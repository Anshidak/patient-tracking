...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'edit') {
        ...
        $stmt_fetch = $conn->prepare("SELECT `Name`, `Nursing_area` FROM `cleaning_table` WHERE `Room`=?");
        $stmt_fetch->bind_param("s", $room);
        $stmt_fetch->execute();
        $stmt_fetch->bind_result($name, $nursingArea);
        $stmt_fetch->fetch();
        $stmt_fetch->close();
        ...
        $log_action = 'UPDATE';
        $stmt_log->bind_param("ssssss", $log_action, $room, $name, $status, $nursingArea, $_SERVER['REMOTE_ADDR']);
        $stmt_log->execute();
        $stmt_log->close();
        ...
        if ($stmt_log === false) {
            die('Error in log query: ' . $conn->error);  // Output MySQL error for the logs query
        }
        $log_action = 'INSERT';
        $stmt_log->bind_param("ssssss", $log_action, $room, $name, $status, $nursingArea, $_SERVER['REMOTE_ADDR']);
        $stmt_log->execute();
        ...
        $log_action = 'DELETE';
        $stmt_log->bind_param("ss", $log_action, $delete_id);
        $stmt_log->execute();
        $stmt_log->close();
    }

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['room']) && isset($_POST['name']) && isset($_POST['status']) && isset($_POST['nursing_area']) && isset($_POST['doctor_name'])) {
		...
		$doctor_name = htmlspecialchars($_POST['doctor_name']); // Get the doctor's name
    ...
    // Edit Status
    if (isset($_POST['edit_id']) && isset($_POST['edit_status']) && isset($_POST['edit_doctor_name'])) {
        $id = $_POST['edit_id'];
        $status = htmlspecialchars($_POST['edit_status']);
		$doctor_name = htmlspecialchars($_POST['edit_doctor_name']); // Get the doctor's name

		$stmt = $conn->prepare("UPDATE cleaning_table SET Status = ?, Doctor_Name = ?, Modified_Date = NOW(), PC_Name = ? WHERE id = ?");
		$stmt->bind_param("sssi", $status, $doctor_name, gethostname(), $id);
		$stmt->execute();
		$stmt->close();
    ...
    <table class="table table-bordered mt-4">
        ...
        <tbody>
            ...
            <tr>
				...
				<td><?php echo htmlspecialchars($row['Doctor_Name']); ?></td> <!-- Display Doctor's Name -->
...
<div class="modal fade" id="editStatusModal" tabindex="-1" role="dialog" aria-labelledby="editStatusModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editStatusModalLabel">Edit Status & Room</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="editStatusForm" method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="edit_room" class="form-control" id="editRoom" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Update Status & Room</button>
                    </div>
                </form>
            </div>
        </div>
